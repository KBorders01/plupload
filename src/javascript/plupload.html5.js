...
], function($, plupload)
{
  ...
  plupload.runtimes.Html5 = plupload.addRuntime("html5", {
    ...
    init : function(uploader, callback) {
      ...
      function addSelectedFiles(native_files, fromdrop) {
        ...
        for (i = 0; i < native_files.length; i++) {
          ...
          // Added fromdrop because all images have the same file name in a web
          // browser on iOS, which prevents adding multiple images at the same
          // time
          if (fileNames[file.name] && fromdrop) {
            continue;
          }
      ...
      uploader.bind("Init", function(up) {
        ...
        inputFile.onchange = function() {
          // Safari on iOS freaks out when stuff is done in this actual event
          // handler, so wait a MS before running anything
          var self=this;
          setTimeout(function(){
            // Add the selected files from file input
            addSelectedFiles(self.files, false);
        ...
        if (browseButton) {        
          ...
          //}
          if (up.features.triggerDialog) {
            $("#"+up.settings.browse_button).on('vclick', function(e){
      ...
      uploader.bind("UploadFile", function(up, file) {
        ...
        function w3cBlobSlice(blob, start, end) {
          ...
          } else if (File.prototype.webkitSlice || File.prototype.mozSlice) {
  ...
  function JPEG_Headers(data) {
    ...
    return {
      ...
      set: function(app, segment) {
        ...
        var i=0;
        var ii=0;
        for (var max = headers.length; i < max; i++) {
          if (headers[i].app === app.toUpperCase()) {
